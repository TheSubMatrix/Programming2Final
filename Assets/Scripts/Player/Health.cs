...
public class Health :MonoBehaviour, IDamagable
{
    ...
    public void Damage(uint damage)
    {
        if(!IsInvulnerable)
        {
            uint newHealth = (uint)Mathf.Clamp((int)CurrentHealth - (int)damage, 0, MaxHealth);
    ...
    {
        uint newHealth = (uint)Mathf.Clamp((int)CurrentHealth + (int)amountToHeal, 0, MaxHealth);;
        CurrentHealth = newHealth;
